...
- **Link engagement metrics — YouTube shipped 2026-05-30; Instagram/Facebook via Meta Graph API (System User token), build in progress 2026-06-23.** `link_metrics` table + 6h cron polls all YouTube links in `report_links` (last 60 days). Views/likes/comments visible in Internal /reports (5th stat card + Top YouTube Links panel) and HR /report (insights panel below form). Provider pattern in `apps/api/src/services/social-insights/` — youtube.provider.ts live; instagram.provider.ts / facebook.provider.ts being filled against the **free Meta Graph API** (NOT a scraper — see the Link Entity Search rule in Key Conventions for why a crawler is rejected: 27k links → IP-ban, login wall, 2GB Linode OOM). `SUPPORTED_INSIGHT_PLATFORMS` in packages/shared is the single switch (currently `["youtube"]`; add `instagram`/`facebook` when the Meta token lands). **Pending UX:** Top YouTube Links panel only shows view count — likes/comments columns and "All time" window option not yet added.
...
- **Why a free crawler is NOT used for IG/FB captions (decided 2026-06-23):** Tempting but rejected for *this* codebase's reality — (1) ~27k IG/FB links from one server IP → Instagram/Facebook **IP-ban** fast; (2) IG/FB serve captions only to **logged-in** sessions now (logged-out crawl hits a login wall; logged-in scrape = ToS violation + account ban); (3) the renderer needed for IG's JS would **OOM the 2GB Linode** that already OOM-kills the `internal` build. The **free Meta Graph API** is both the permanent AND quickest reliable path — the caption comes in the SAME call as engagement metrics. **Opaque Facebook links** (`facebook.com/share/r/<code>` — ~84% / 6,121 of FB links) carry a redirect *share token*, not a post ID, and redirect to an equally-opaque `pfbid…` permalink the Graph API can't query; clean `/reel/<numericId>` (~1,188) IS resolvable. Plan: ship IG (~18,200 clean shortcodes — the big win) + clean FB, attempt a **cheap best-effort HEAD-redirect** on opaque links (recovers only those redirecting to a clean numeric URL, gives up on `pfbid` — no fragile feed-matcher), coverage banner stays honest. Full analysis: `.planning/FB-INSTAGRAM-INSIGHTS-PLAN.md`.
