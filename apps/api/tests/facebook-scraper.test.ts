...
describe("parseFbReelHtml", () => {
  ...
  it("returns all-null for a body shorter than the reel-page minimum (login wall / shell)", () => {
    ...
    expect(parseFbReelHtml(wall)).toEqual({ views: null, likes: null, comments: null });
  });

  it("returns all-null for empty input", () => {
    expect(parseFbReelHtml("")).toEqual({ views: null, likes: null, comments: null });
  });
  ...
  it("handles a reel with zero comments and present reactions", () => {
    ...
    expect(r).toEqual({ views: 4455, likes: 78, comments: 0 });
  });
...
describe("scrapeFacebookReelEngagement", () => {
  it("rejects non-numeric ids without any fetch", async () => {
    ...
    expect(r).toEqual({ views: null, likes: null, comments: null });
  });

  it("parses engagement from a 200 reel response", async () => {
    const html = reelHtml({ videoViewCount: 9859, reactionCount: 198, totalCommentCount: 0 });
    ...
    expect(r).toEqual({ views: 9859, likes: 198, comments: 0 });
  });
