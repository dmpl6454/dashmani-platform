...
- **Offer Letter / Appointment Letter "Send to Employee":** Likely needs the same fix as the Contract generator (see Send-to-Employee fix below). Both AI doc generators currently produce HTML previews only — verify whether their save endpoints are wired before treating any "employee can't see my offer letter" report as a new bug.
