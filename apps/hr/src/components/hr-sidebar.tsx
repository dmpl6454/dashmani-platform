...
type NavItem = { href: string; label: string; icon: React.ElementType; group?: string; badgeKey?: string };
...
function MoreGrid({ collapsed, pathname }: { collapsed: boolean; pathname: string }) {
...
export function HrSidebar() {
  ...
  return (
    <aside
      className={`${collapsed ? "w-[58px]" : "w-[220px]"} shrink-0 flex flex-col bg-surface transition-[width] duration-200`}
      style={{ borderRight: "2px solid rgba(26,26,26,0.09)" }}
    >
      {/* Logo */}
      <div
        className={`flex items-center ${collapsed ? "justify-center px-0" : "gap-3 px-4"} h-16 shrink-0`}
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto min-h-0">
        {NAV_MAIN.map((n) => {
          ...
          const I = n.icon;
          const showGroup = !collapsed && n.group && n.group !== lastGroup;
          ...
          return (
            <div key={n.href}>
              ...
              <Link href={n.href} title={collapsed ? n.label : undefined}
                className={`w-full flex items-center ${collapsed ? "justify-center px-0" : "gap-3 px-3"} h-10 rounded-xl text-[13px] transition-all duration-150 relative mb-0.5 ${
        ...
        {/* More grid */}
        <MoreGrid collapsed={collapsed} pathname={pathname} />
      </nav>
      ...
      <div className="px-2 py-3 shrink-0 space-y-0.5" style={{ borderTop: "2px solid rgba(26,26,26,0.07)" }}>
        ...
        <div className={`w-full flex items-center ${collapsed ? "justify-center px-0" : "gap-3 px-2"} h-12 rounded-xl hover:bg-muted/70 transition-colors`}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0 leading-tight">
              <div className="text-[12.5px] font-bold text-ink truncate">{user?.name}</div>
              <div className="text-[10.5px] text-ink-3 font-medium truncate">{user?.email}</div>
            </div>
          )}
        </div>
