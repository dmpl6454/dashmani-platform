...
export default function AdminLeaderboard() {
  const { data, loading, refreshing, error, refresh } = useApi<any[]>(() => apiFetch("/admin/reports/leaderboard"));
  ...
  return (
    <Screen onRefresh={refresh} refreshing={refreshing}>
      ...
      {rows.length === 0 ? (
        <Card>
          <Empty icon="trophy-outline" text="No leaderboard data" />
        </Card>
      ) : (
        <Card>
          {rows.map((e: any, i: number) => {
            ...
            return (
              <View key={e.employee?.id ?? i} style={[styles.row, i === rows.length - 1 && { borderBottomWidth: 0 }]}>
