...
- **Real-time link engagement metrics (future, planned-not-started):** Employees paste social media links in daily reports but likes/comments/views aren't tracked, so a meaningful employee growth-rate stat can't be computed. Each platform has its own auth ceiling: Instagram/Facebook require Business-account authorization per employee; YouTube Data API is the easiest (free, video ID lookup); TikTok requires creator auth; X is paid only. ~2–3 week build (auth flows + cron + metrics table + UI). Worth a planning doc when prioritized.
...
- **Salary slips flow (planned, not shipped):** Current state is broken in two ways: (a) [apps/internal/src/app/salary-slips/page.tsx](apps/internal/src/app/salary-slips/page.tsx) renders `slip.basic` and `slip.employeeName` but the API returns `slip.basicSalary` and `slip.employee.name` — so the Basic column and sometimes the name column show `—` even when slips exist; the search input is also wired client-side but `listSalarySlips()` in [salary-slip.service.ts](apps/api/src/services/salary-slip.service.ts) ignores the `search` param. (b) The "Salary Slips" tab inside AI Assistant ([apps/internal/src/app/ai-assistant/page.tsx](apps/internal/src/app/ai-assistant/page.tsx) `SalarySlipViewer`) is a read-only duplicate of the listing page — it has no employee picker, no editable fields, no Send-to-Employee button, breaking the Offer/Contract/Appointment muscle memory. There is also no UI to create a single salary slip (only bulk-generate exists) and no way to edit values before approval. **Full fix plan: [`.planning/SALARY-SLIPS-FIX-PLAN.md`](.planning/SALARY-SLIPS-FIX-PLAN.md)** — replaces the AI Assistant viewer with a proper `SalarySlipGenerator` (calls existing `POST /admin/salary-slips/generate` after preview), adds `PUT /admin/salary-slips/:id` + Edit modal on the dedicated page, adds `search` filter to `listSalarySlips`, fixes the field-name bugs. 5 files, ~300 LOC, no schema change, no `db:push`. Same "orphan generator" pattern that bit us on offer/contract — see [feedback_orphan_admin_contracts_endpoint](memory link). The HR portal employee view at [apps/hr/src/app/salary-slips/page.tsx](apps/hr/src/app/salary-slips/page.tsx) already works and needs no change.
